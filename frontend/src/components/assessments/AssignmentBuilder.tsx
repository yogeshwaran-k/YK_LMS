...
export default function AssignmentBuilder({ assessment, onBack }: AssignmentBuilderProps) {
  ...
  async function fetchAssignment() {
    setLoading(true);
    try {
      const data = await api.get<Assignment | null>(`/assessments/${assessment.id}/assignment`);
      if (data) setAssignment(data); else setEditing(true);
    } finally {
      setLoading(false);
    }
  }
...
function AssignmentForm({ assessmentId, assignment, onSuccess, onCancel }: AssignmentFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (assignment) {
        await api.put(`/assessments/assignment/${assignment.id}`, assignmentData);
      } else {
        await api.post(`/assessments/${assessmentId}/assignment`, assignmentData);
      }

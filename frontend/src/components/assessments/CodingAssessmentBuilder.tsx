...
export default function CodingAssessmentBuilder({ assessment, onBack }: CodingAssessmentBuilderProps) {
  ...
  async function fetchQuestions() {
    ...
    try {
      ...
      setQuestions(data);
      if (data.length > 0 && !selectedQuestion) setSelectedQuestion(data[0]);
    } finally {
      setLoading(false);
    }
  }
              ...
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="mt-4 bg-white rounded-lg border p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="text-sm font-semibold text-gray-900">Reference Solution (admin)</h3>
                    <select value={referenceLanguage} onChange={(e)=>setReferenceLanguage(e.target.value as any)} className="text-sm border rounded px-2 py-1">
                      {(['javascript','python','cpp','c','java','typescript'] as const).map(l => <option key={l} value={l}>{l}</option>)}
                    </select>
                    <button onClick={()=>setReferenceSolution(selectedQuestion.starter_code||'')} className="text-sm px-2 py-1 border rounded">Use starter</button>
                  </div>
                  <AceCodeEditor value={referenceSolution} onChange={setReferenceSolution} language={referenceLanguage} height="220px" />
                </div>
              </div>
...
function TestCaseModal({ questionId, testCase, onClose, onSuccess, referenceSolution, referenceLanguage }: TestCaseModalProps & { referenceSolution?: string; referenceLanguage?: string }) {
  const [formData, setFormData] = useState({
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              ...
              onClick={async ()=>{
                try {
                  const res = await api.post<{ stdout: string }>(`/runner/execute`, { language: referenceLanguage || 'javascript', code: referenceSolution || '', stdin: (formData as any).input || '' });

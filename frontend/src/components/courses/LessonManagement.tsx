...
export default function LessonManagement({ courseId, moduleId, moduleName, onBack }: LessonManagementProps) {
  ...
  async function fetchLessons() {
    setLoading(true);
    try {
      const data = await api.get<Lesson[]>(`/courses/${courseId}/modules/${moduleId}/lessons`);
      setLessons(data);
    } finally {
      setLoading(false);
    }
...
function LessonModal({ courseId, moduleId, lesson, nextOrderIndex, onClose, onSuccess }: LessonModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const payload: NewLessonPayload = {
        title: formData.title,
        content_type: formData.content_type,
        min_time_minutes: formData.min_time_minutes,
      };
      if (formData.content_type === 'text') {
        if (formData.content_text && formData.content_text.trim().length > 0) {
          payload.content_text = formData.content_text;
        }
      } else {
        if (formData.content_url && formData.content_url.trim().length > 0) {
          payload.content_url = formData.content_url;
        }
      }
      if (lesson) {
        await api.put(`/courses/${courseId}/modules/${moduleId}/lessons/${lesson.id}`, payload);
      } else {
        payload.order_index = nextOrderIndex;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, content_type: e.target.value as 'text' | 'video' | 'pdf' | 'ppt' | 'coding' })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
